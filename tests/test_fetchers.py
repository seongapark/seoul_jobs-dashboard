...
def _renamed_sido_grid(period):
    ...
    예시 이름은 **우리가 모르는** 표기여야 한다. 예전에는 '서울특별시'·'경기도'·
    '인천광역시' 를 썼는데, 실측(2026-09-03)이 그 정식명이 (사업장)시도 축에서
    실제로 쓰이는 표기임을 밝혀 `_SIDO_NAME_TO_CODE` 에 들어갔다 — 그래서 이제는
    "모르는 라벨"의 예시로 쓸 수 없다. 가정이 실측에 반증되면 예시를 바꾼다.
    """
    label = f"{period[:4]}년 {period[4:]}월"
    rows = [{"(근무지역)시도": name,
             f"{label}_유효구인인원(전체)": "10",
             f"{label}_유효구직자수(전체)": "100"}
            for name in ("서울시", "경기 특별자치도", "인천 광역시")]
    return olap.ParsedGrid(rows, [])
