...
반기 수집을 **사람이 직접** 돌릴 때는 기간을 지정할 수 있습니다:

```bash
python -m pipeline.cli halfyear 202601
```

지정하지 않으면 실행 월로 기간을 정합니다(1~6월 → 그 해 상반기 `YYYY01`,
7~12월 → 하반기 `YYYY02`). 이 기본값은 워크플로의 6월 20일·12월 20일 실행에
맞춘 것이라, **그 밖의 시점에 수동으로 돌리면 아직 공표되지 않은 반기를
요청할 수 있습니다**(실측 2026-09-03: `202602` 는 "데이터가 존재하지 않습니다",
`202601` 은 정상). 그럴 때 기간을 직접 주십시오.

GitHub Actions 에서 수동 실행할 때도 `period` 입력에 같은 값을 넣을 수 있습니다
(비워 두면 실행 월로 정합니다).
