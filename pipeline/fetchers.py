...
# 넷 중 **41283 하나뿐**이고, 나머지 셋이 빠진 것은 관측을 못 해서가 아니라
# 구조적으로 후보가 될 수 없어서다 (최종 브랜치 리뷰의 논증):
#
#   - 41283 경기도 고양시 일산구 (2005 폐지, 일산동구·일산서구로 분할). 모시 후보
#     '경기도 고양시' 가 sigungu_names.json 에 41280 으로 실재하므로, 명시하지
#     않으면 EIS 가 그 라벨을 내는 순간 고양시에 조용히 합산된다. 그래서 여기 있다.
#   - 41710·41730·41810 은 **일반구가 아니라 폐지된 시·군이다.** 행정표준코드에서
#     일반구는 언제나 *모시코드 + 1..n* 이라 끝자리가 0 이 아니고(41111=41110+1,
#     41283=41280+3, 41135=41130+5), 시·군 레벨 코드는 전부 0 으로 끝난다 —
#     이 저장소의 경기 코드 31개가 전부 0 으로 끝나는 것으로 확인된다. 문제의 셋은
#     0 으로 끝나므로 시·군이고, 시·군 라벨은 낱말이 둘("경기도 ○○시")이라
#     `_reparent_general_districts` 의 `len(parts) < 3` 에서 **이관 후보가 애초에
#     안 된다**. 모시 후보가 시도 이름('경기도')이 돼 `sigungu_names.json` 에도
#     없으므로, 지금 코드가 이미 옳게 처리한다(그냥 걸러진다).
#
# 즉 이 목록에 이름을 더 채워야 하는 열린 위험은 없다.
...
def _reparent_general_districts(rows: list[dict], spec: Spec) -> tuple[list[dict], bool]:
    """경기 일반구 행을 모시(母市) 이름으로 바꿔 합산 이관한다 (R53).

    `(rows, reparented)` 를 돌려준다 — `reparented` 는 실제로 한 행이라도 옮겼는지다.
    `_merge_rows(merge_expected=...)` 가 그 값을 받아 "합칠 정상 중복이 있을 수
    있는가"를 판단한다: 이관이 없었는데 축이 같은 행이 둘이면 그건 정상 중복이
    아니라 그리드 중복·집계 행 누출이므로 조용히 더하지 않고 예외를 낸다.
