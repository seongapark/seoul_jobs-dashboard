"""EIS OLAP 기본 레이아웃((지역별)시도 x 마감년월)을 시계열 행으로 편다.

Task 9b (R19/R27) — 플랜에 없던 태스크였다. §4.1 카드 2(24개월 추세),
§4.2 카드 8·§4.3 카드 11(이번 달 대 최근 6개월 평균)이 이력을 요구하는데
`pipeline/eis.py` 의 `collect_*_sido` 는 최신 한 달치만 편다. Task 6 실측
으로 유효구인구직 리포트의 **기본 레이아웃**이 이미 (지역별)시도 x 마감년월
이라는 게 확인됐으므로, 축을 드래그로 옮기지 않고 기본 레이아웃 그대로
받으면 여러 달치가 한 번에 온다 — 그래서 "매달 쌓기"가 아니라 "한 번에
백필"이다(R27).
